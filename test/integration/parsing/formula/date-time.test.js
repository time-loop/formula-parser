...
describe('.parse() date & time formulas', () => {
    it.each([new Parser(), ClickUpParser.create()])('DATE', (parser) => {
        expect(parser.parse('DATE()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DATE(null, 5, 12)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DATE(2001, null, 12)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DATE(2001, 5, null)')).toMatchObject({ error: null, result: Number.NaN });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DATEVALUE', (parser) => {
        expect(parser.parse('DATEVALUE()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DATEVALUE(null)')).toMatchObject({ error: null, result: Number.NaN });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DAY', (parser) => {
        expect(parser.parse('DAY()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('DAY("2958465")')).toMatchObject({ error: null, result: 31 });
        expect(parser.parse('DAY(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('DAYS', (parser) => {
        expect(parser.parse('DAYS()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS(1)')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('DAYS(null, 1)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS(1, null)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS(null, null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('DAYS360', (parser) => {
        expect(parser.parse('DAYS360()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS360(1)')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('DAYS360(null, "1/1/1901")')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS360("1/1/1901", null)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('DAYS360(null, null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('EDATE', (parser) => {
        expect(parser.parse('EDATE()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('EDATE(1)')).toMatchObject({ error: null, result: 1 });
        expect(parser.parse('EDATE("1/1/1900", 1)')).toMatchObject({ error: null, result: 32 });
        expect(parser.parse('EDATE(null, 1)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('EDATE("1/1/1900", null)')).toMatchObject({ error: null, result: 1 });
        expect(parser.parse('EDATE(null, null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('EOMONTH', (parser) => {
        expect(parser.parse('EOMONTH()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('EOMONTH(1)')).toMatchObject({ error: null, result: 31 });
        expect(parser.parse('EOMONTH("1/1/1900", 1)')).toMatchObject({ error: null, result: 59 });
        expect(parser.parse('EOMONTH(null, 1)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('EOMONTH("1/1/1900", null)')).toMatchObject({ error: null, result: 31 });
        expect(parser.parse('EOMONTH(null, null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('HOUR', (parser) => {
        expect(parser.parse('HOUR()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('HOUR("1/1/1900 16:33")')).toMatchObject({ error: null, result: 16 });
        expect(parser.parse('HOUR(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('INTERVAL', (parser) => {
        expect(parser.parse('INTERVAL()')).toMatchObject({ error: null, result: 'PT' });
    ...
    it.each([new Parser(), ClickUpParser.create()])('ISOWEEKNUM', (parser) => {
        expect(parser.parse('ISOWEEKNUM()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('ISOWEEKNUM(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('MINUTE', (parser) => {
        expect(parser.parse('MINUTE()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('MINUTE(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('MONTH', (parser) => {
        expect(parser.parse('MONTH()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('MONTH(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('NETWORKDAYS', (parser) => {
        expect(parser.parse('NETWORKDAYS()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('NETWORKDAYS("2/1/1901")')).toMatchObject({ error: null, result: Number.NaN });
    ...
    it.each([new Parser(), ClickUpParser.create()])('SECOND', (parser) => {
        expect(parser.parse('SECOND()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('SECOND("2/1/1901 13:33:12")')).toMatchObject({ error: null, result: 12 });
        expect(parser.parse('SECOND(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('TIME', (parser) => {
        expect(parser.parse('TIME()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('TIME(0)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('TIME(0, 0)')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('TIME(null, 0, 0)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('TIME(0, null, 0)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('TIME(0, 0, null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('TIMEVALUE', (parser) => {
        expect(parser.parse('TIMEVALUE()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('TIMEVALUE(null)')).toMatchObject({ error: null, result: Number.NaN });
    });
    ...
    it.each([new Parser(), ClickUpParser.create()])('WEEKDAY', (parser) => {
        expect(parser.parse('WEEKDAY()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('WEEKDAY(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('WEEKNUM', (parser) => {
        expect(parser.parse('WEEKNUM()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('WEEKNUM(null)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('WEEKNUM("a")')).toMatchObject({ error: '#VALUE!', result: null });
    });

    it.each([new Parser(), ClickUpParser.create()])('WORKDAY', (parser) => {
        expect(parser.parse('WORKDAY()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('WORKDAY("1/1/1900")')).toMatchObject({ error: null, result: new Date(1900, 0, 1) });
        expect(parser.parse('WORKDAY(null)')).toMatchObject({ error: null, result: Number.NaN });
    ...
    it.each([new Parser(), ClickUpParser.create()])('YEAR', (parser) => {
        expect(parser.parse('YEAR()')).toMatchObject({ error: null, result: Number.NaN });
        ...
        expect(parser.parse('YEAR(null)')).toMatchObject({ error: null, result: Number.NaN });
    });

    it.each([new Parser(), ClickUpParser.create()])('YEARFRAC', (parser) => {
        expect(parser.parse('YEARFRAC()')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('YEARFRAC("1/1/1904")')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('YEARFRAC("1/1/1900", "1/2/1900")')).toMatchObject({
            error: null,
            result: 0.002777777777777778,
        });
        expect(parser.parse('YEARFRAC(null, "1/2/1900")')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('YEARFRAC("1/1/1900", null)')).toMatchObject({ error: null, result: Number.NaN });
        expect(parser.parse('YEARFRAC(null, null)')).toMatchObject({ error: null, result: Number.NaN });
    });
});
