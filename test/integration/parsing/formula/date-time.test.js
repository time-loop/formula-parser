...
describe('.parse() date & time formulas', () => {
    it.each([new Parser(), ClickUpParser.create()])('DATE', (parser) => {
        expect(parser.parse('DATE()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DATEVALUE', (parser) => {
        expect(parser.parse('DATEVALUE()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DAY', (parser) => {
        expect(parser.parse('DAY()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DAYS', (parser) => {
        expect(parser.parse('DAYS()')).toMatchObject({ error: null, result: null });
        expect(parser.parse('DAYS(1)')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('DAYS360', (parser) => {
        expect(parser.parse('DAYS360()')).toMatchObject({ error: null, result: null });
        expect(parser.parse('DAYS360(1)')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('EDATE', (parser) => {
        expect(parser.parse('EDATE()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('EOMONTH', (parser) => {
        expect(parser.parse('EOMONTH()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('HOUR', (parser) => {
        expect(parser.parse('HOUR()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('ISOWEEKNUM', (parser) => {
        expect(parser.parse('ISOWEEKNUM()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('MINUTE', (parser) => {
        expect(parser.parse('MINUTE()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('MONTH', (parser) => {
        expect(parser.parse('MONTH()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('NETWORKDAYS', (parser) => {
        expect(parser.parse('NETWORKDAYS()')).toMatchObject({ error: null, result: null });
        expect(parser.parse('NETWORKDAYS("2/1/1901")')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('SECOND', (parser) => {
        expect(parser.parse('SECOND()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('TIME', (parser) => {
        expect(parser.parse('TIME()')).toMatchObject({ error: null, result: null });
        expect(parser.parse('TIME(0)')).toMatchObject({ error: null, result: null });
        expect(parser.parse('TIME(0, 0)')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('TIMEVALUE', (parser) => {
        expect(parser.parse('TIMEVALUE()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('WEEKDAY', (parser) => {
        expect(parser.parse('WEEKDAY()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('WEEKNUM', (parser) => {
        expect(parser.parse('WEEKNUM()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('WORKDAY', (parser) => {
        expect(parser.parse('WORKDAY()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('YEAR', (parser) => {
        expect(parser.parse('YEAR()')).toMatchObject({ error: null, result: null });
    ...
    it.each([new Parser(), ClickUpParser.create()])('YEARFRAC', (parser) => {
        expect(parser.parse('YEARFRAC()')).toMatchObject({ error: null, result: null });
        expect(parser.parse('YEARFRAC("1/1/1904")')).toMatchObject({ error: null, result: null });
